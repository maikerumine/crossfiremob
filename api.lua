crossfiremob = {}
crossfiremob.mod = "crossfiremob"
function crossfiremob:register_mob(name, def)
	minetest.register_entity(name, {
		...
		--SOUND  ATTACK
			...
			on_step = function(self, dtime)
					...
					if self.lifetimer <= 0 and not self.tamed and self.type ~= "goodnpc" then
			...
			-- FIND SOMEONE TO ATTACK
				if ( self.type == "player" or self.type == "monster" ) and minetest.setting_getbool("enable_damage") and self.state ~= "attack" then
			...
			-- BLACK FIND A BADPLAYER OR GOODNPC TO ATTACK
				 if self.type == "badnpc" and self.attacks_monsters and self.state ~= "attack" then
					 local s = self.object:getpos()
					 local inradius = minetest.get_objects_inside_radius(s,self.view_range)
					 for _, oir in pairs(inradius) do
						 local obj = oir:get_luaentity()
						 if obj then
							 if obj.type == "player" or obj.type == "goodnpc" then
								-- attack white
			...
			-- WHITE FIND A BADPLAYER OR WHITE TO ATTACK
				 if self.type == "goodnpc" and self.attacks_monsters and self.state ~= "attack" then
					 local s = self.object:getpos()
					 local inradius = minetest.get_objects_inside_radius(s,self.view_range)
					 for _, oir in pairs(inradius) do
						 local obj = oir:get_luaentity()
						 if obj then
							 if obj.type == "player" or obj.type == "badnpc" then
								 -- attack black
								 local p = obj.object:getpos()
								 local dist = ((p.x-s.x)^2 + (p.y-s.y)^2 + (p.z-s.z)^2)^0.5
								 self.do_attack(self,obj.object,dist)
	...
	on_activate = function(self, staticdata, dtime_s)
		...
		if self.type == "badbnpc" and minetest.setting_getbool("only_peaceful_mobs") then
			self.object:remove()
		end
			...
			if hitter and hitter:is_player() then -- and hitter:get_inventory() then
				for _,drop in ipairs(self.drops) do
					if math.random(1, drop.chance) == 1 then
						...
						local pos2 = pos
						pos2.y = pos2.y + 1.5 -- drop items one block higher
...
--SPAWNING CODE
crossfiremob.spawning_mobs = {}
function crossfiremob:register_spawn(name, nodes, max_light, min_light, chance, active_object_count, max_height, min_dist, max_dist, spawn_func)
		crossfiremob.spawning_mobs[name] = true
		minetest.register_abm({
			...
			action = function(pos, node, _, active_object_count_wider)
				...
				if not crossfiremob.spawning_mobs[name] then
				return
				end
				pos.y = pos.y+1
